...
# transform the genotype with Gaussian kernel, running in GPU
# def rbf_kernel_new(X, Y=None, gamma=None):
#     X = cp.asarray(X)
#     if gamma is None:
#         gamma = 1 / (X.shape[1])
#     if Y is None:
#         pairwise_dists = cp.sum(X ** 2, axis=1).reshape(-1, 1) + cp.sum(X ** 2, axis=1) - 2 * cp.dot(X, X.T)
#         kernel_matrix = cp.exp(-gamma * pairwise_dists)
#         return kernel_matrix.get()
#     else:
#         Y = cp.asarray(Y)
#         pairwise_dists = cp.sum(X ** 2, axis=1).reshape(-1, 1) + cp.sum(Y ** 2, axis=1) - 2 * cp.dot(X, Y.T)
#         kernel_matrix = cp.exp(-gamma * pairwise_dists)
#         return kernel_matrix.get()
...
def interact_gene_pre(assist_gene, target_gene, ger, factor=1000):
