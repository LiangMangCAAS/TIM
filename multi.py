...
parser.add_argument("--output", type=str, default="TMP/preMatrix_multi")
...
def multi_funB(i):
    ...
    x, x_pre = interact_gene_pre(assist_gene, gene, ger, predict=predict)
    acc, best_alpha, pre = tune_hp(x.values, y, factor=100)
    original_acc =  gene_params.iloc[:, 0].loc[gene]
    if (acc > original_acc) and (predict=="True"):
        # print((acc > original_acc) and (predict=="True"))
        ...
        t = time.localtime()
        t = time.asctime(t)
        ge_pre.to_pickle('{}/{}_{}'.format(out_put, gene, t))
    elif acc < original_acc:
        acc = original_acc
    # if i % 100 == 0:
    #     t = time.localtime()
...
if __name__ == '__main__':
    ...
    t = time.localtime()
    t = time.asctime(t)
    # res1.to_excel('results/multi_accuracy_{}.xlsx'.format(t))
